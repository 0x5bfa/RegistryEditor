...
function Save-StartupDiagnostics
{
    ...
    if ($null -ne $InstalledPackage)
    {
        $appLogPath = Join-Path `
            $env:LOCALAPPDATA `
            "Packages\$($InstalledPackage.PackageFamilyName)\LocalState\RegistryEditor.Logs\Log.log"

        if (Test-Path -LiteralPath $appLogPath -PathType Leaf)
        {
            Copy-Item `
                -LiteralPath $appLogPath `
                -Destination (Join-Path $diagnosticsDirectory "RegistryEditor.Log.log") `
                -Force
        }
    }
}
...
try
{
    ...
    if (-not ("RegistryEditor.CI.ApplicationActivation" -as [type]))
    {
        Add-Type -TypeDefinition @"
using System;
using System.Runtime.InteropServices;

namespace RegistryEditor.CI
{
    [ComImport]
...
"@
    ...
    $launchStartTime = [DateTime]::UtcNow
    $processId = [RegistryEditor.CI.ApplicationActivation]::Activate($applicationUserModelId)
